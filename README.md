...
The private AI consent page now exposes separate write-permission management and exact proposal review, behind AI_CONNECTOR_ENABLED. Bearer-only status/prepare/publish routes are distinct from session-and-origin-protected authorize/review/approve routes. Only an opaque review ID enters the source review link; proposal content stays out of URLs. Consent allows one selected destination and notes/tasks; each proposal still requires source-user approval. Existing read-only connections do not gain write permission automatically.
